...
const ZipFieldSelector = ({ open, onClose, onGenerate }) => {
  const [selectedFields, setSelectedFields] = useState([]);
  const [selectAll, setSelectAll] = useState(false);

  // Module configuration matching backend MODULE_CONFIG
  const modules = [
    {
  ...
  return (
    ...
    >
      <DialogTitle sx={{ pb: 1, backgroundColor: '#1a237e', color: 'white' }}>
        <Typography variant="h6" fontWeight="bold">
          Select Modules for ZIP Download
        </Typography>
        <Typography variant="body2" sx={{ mt: 0.5, opacity: 0.9 }}>
          Choose which module documents to include in the ZIP file
        </Typography>
      </DialogTitle>
      ...
      <DialogContent sx={{ pt: 3 }}>
        ...
        >
          <FormControlLabel
            ...
            label={
              <Typography variant="body1" fontWeight="600" color="#1565c0">
                Select All Modules
              </Typography>
            }
          />
...
export default ZipFieldSelector;

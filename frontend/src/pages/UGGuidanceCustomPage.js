...
const UGGuidanceCustomPage = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`/api/ug-guidance?termId=${selectedTerm}`);
      const itemsData = response.data?.data || response.data || [];
      console.log('Fetched UG Guidance data:', itemsData);
  ...
  const handleClose = () => {
    setOpen(false);
    setFormData({ numberOfStudents: 0, projectTitle: '', remarks: '' });
    setStudents([]);
    setUploadedFiles([]);
    setError('');
    setCurrentItemId(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editMode) {
        const response = await axios.put(`/api/ug-guidance/${currentItemId}`, submitData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
        console.log('Update response:', response.data);
        setSuccess('Record updated successfully');
      } else {
        const response = await axios.post(`/api/ug-guidance`, submitData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      try {
        await axios.delete(`/api/ug-guidance/${id}`);
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
      ...
      <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
        <DialogTitle>
          {editMode ? 'Edit UG Research Guidance' : 'Add New UG Research Guidance'}
        </DialogTitle>
        <DialogContent>
          <Box component="form" id="ug-guidance-form" onSubmit={handleSubmit} pt={1}>
            <TextField
              fullWidth
              margin="normal"
...
export default UGGuidanceCustomPage;

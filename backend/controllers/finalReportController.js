...
const { generateAppraisalPDF } = require('../utils/pdfGenerator');
...
/**
 * Generate Final Report with ZIP containing all files and PDF
...
exports.generateFinalReport = async (req, res) => {
  try {
    ...
    // Generate PDF
    const pdfBuffer = await generateAppraisalPDF(faculty, term, appraisalData);
    ...
    const zipFileName = `${faculty.name.replace(/\s+/g, '_')}_${term.termName.replace(/\s+/g, '_')}.zip`;
    ...
    // Add PDF to ZIP
    archive.append(pdfBuffer, { name: 'Appraisal_Report.pdf' });
      ...
      if (data && data.length > 0) {
        for (const item of data) {
          if (item.filePath && fs.existsSync(item.filePath)) {
            ...
            archive.file(item.filePath, { name: `${category.folder}/${fileName}` });
          }
        }
      }
    }
